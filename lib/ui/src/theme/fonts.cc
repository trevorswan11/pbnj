...
auto font_manager::init(f32 dpi_scale) noexcept -> void {
    ...
    reg_body_       = *add_font(assets::inter_regular_18pt, 18 * dpi_scale, cfg);
    bold_body_      = *add_font(assets::inter_bold_18pt, 15 * dpi_scale, cfg);
    italic_body_    = *add_font(assets::inter_italic_18pt, 18 * dpi_scale, cfg);
    caption_        = *add_font(assets::inter_regular_18pt, 12 * dpi_scale, cfg);
    italic_caption_ = *add_font(assets::inter_italic_18pt, 12 * dpi_scale, cfg);
    subheading_     = *add_font(assets::inter_bold_24pt, 20 * dpi_scale, cfg);
    heading_        = *add_font(assets::inter_bold_28pt, 26 * dpi_scale, cfg);
    title_          = *add_font(assets::inter_bold_28pt, 34 * dpi_scale, cfg);
...
auto font_manager::add_font(gsl::span<unsigned char> raw_ttf,
    ...
    return io.Fonts->AddFontFromMemoryTTF(
        raw_ttf.data(), static_cast<i32>(raw_ttf.size()), font_size, &cfg);
}
